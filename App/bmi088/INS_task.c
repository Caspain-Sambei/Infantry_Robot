...
void INS_task_Init(void)
{
    while(BMI088_init()) {};
    // while(ist8310_init()) {};
    BMI088_read(p_reg->bmi088_real_data.gyro, p_reg->bmi088_real_data.accel, &p_reg->bmi088_real_data.temp);
    ...
    AHRS_init(INS_quat, p_reg->bmi088_real_data.accel, NULL);
...
void INS_task(void)
{
    if(gyro_update_flag & (1 << IMU_NOTIFY_SHFITS))
    {
        ...
        BMI088_gyro_read_over(gyro_dma_rx_buf + BMI088_GYRO_RX_BUF_DATA_OFFSET, p_reg->bmi088_real_data.gyro);
    }

    if(accel_update_flag & (1 << IMU_UPDATE_SHFITS))
    {
        accel_update_flag &= ~(1 << IMU_UPDATE_SHFITS);
        BMI088_accel_read_over(accel_dma_rx_buf + BMI088_ACCEL_RX_BUF_DATA_OFFSET, p_reg->bmi088_real_data.accel, &p_reg->bmi088_real_data.time);
    }

    if(accel_temp_update_flag & (1 << IMU_UPDATE_SHFITS))
    {
        accel_temp_update_flag &= ~(1 << IMU_UPDATE_SHFITS);
        BMI088_temperature_read_over(accel_temp_dma_rx_buf + BMI088_ACCEL_RX_BUF_DATA_OFFSET, &p_reg->bmi088_real_data.temp);
        imu_temp_control(p_reg->bmi088_real_data.temp);
    }

    // AHRS_update(INS_quat, 0.001f, bmi088_real_data.gyro, bmi088_real_data.accel, ist8310_real_data.mag);
    AHRS_update(INS_quat, 0.001f, p_reg->bmi088_real_data.gyro, p_reg->bmi088_real_data.accel, NULL);
