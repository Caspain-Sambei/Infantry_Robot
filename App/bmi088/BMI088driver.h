...
#define BMI088_COM_WAIT_SENSOR_TIME 150	// 传感器操作后的最大等待超时（150ms）
...
typedef struct BMI088_RAW_DATA
{
    ...
    int16_t accel[3];	//加速度计X/Y/Z原始值（对应ACC_X/Y/Z寄存器）
    int16_t temp;		//温度计原始值（对应ACC_TEMP0/1寄存器）
    int16_t gyro[3];	//陀螺仪原始值（对应GYRO_X/Y/Z寄存器）
...
typedef struct BMI088_REAL_DATA
{
    ...
    fp32 temp;
    fp32 gyro[3];
    fp32 time;	//对应 BMI088 的时间戳寄存器（ACC_SENSOR_TIME0/1/2），用于精准同步数据。
} bmi088_real_data_t;

// 每个错误码对应 “寄存器配置 / 自检” 的一个环节，初始化函数返回这些错误码，方便定位硬件问题。
enum
{
    BMI088_NO_ERROR                     = 0x00,	// 无错误
    BMI088_ACC_PWR_CTRL_ERROR           = 0x01,	// 加速度计电源控制寄存器配置失败
    BMI088_ACC_PWR_CONF_ERROR           = 0x02,	// 加速度计电源配置寄存器失败
    BMI088_ACC_CONF_ERROR               = 0x03,	// 加速度计配置寄存器（量程/带宽）失败
...
//自检：写入 ACC_SELF_TEST或GYRO_SELF_TEST寄存器，读取反馈值判断硬件是否正常
...
//数据解析：将寄存器原始值（int16_t）× 灵敏度系数，转为物理值（fp32）
...
//读陀螺仪 WHO_AM_I 寄存器（固定值 0x0F），验证传感器是否存在
extern void BMI088_read_gyro_who_am_i(void);
//读加速度计 WHO_AM_I 寄存器（固定值 0x1E），验证传感器是否存在
